...
菜单里每一项都能单独关（见下）。**默认窗口在前台也通知** —— 「跑完了」这件事本身就有信息量，
不该因为你在看别的地方就被吞掉；想要安静可以切到「仅窗口不在前台时」。
...
- **启用通知** —— 总开关
- **通知方式** —— `始终通知` / `仅窗口不在前台时` / `仅长任务（超过 30 秒）`
- **中断时也通知** —— 一轮被 Esc 打断算不算
- **通知内容** —— 显示会话标题 / 显示用时 / 显示回复摘要
- **测试通知** —— 真发一条，把投递结果（成功 / 被拦 / 原因）用对话框告诉你
- **最近投递** —— 只读，显示最后一次通知的结果
