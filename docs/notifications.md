...
对话结束或需要你操作时弹系统通知 —— 不往 dsh 里加任何插件，靠 tail 会话事件流实现。

---

网页版最大的问题是「跑长任务时只能盯着标签页」。桌面壳补上了这个：**对话完成、中断、等待人工选择或等待授权时发一条系统通知，点一下回到窗口**。
...
```
$DSH_HOME/sessions/<编码后的 cwd>/<session-id>/session.v3.jsonl.zstd
```

这是 zstd 分帧的追加式日志，每帧是一批事件（`turn/start`、`assistant/message`、`turn/end` …）。
`notify.js` 优先 tail 新版 `session.v3.jsonl.zstd`，并兼容旧版 `session.jsonl.zstd`；同一会话只读一份，避免升级过渡期重复通知。
...
| 状态 | 事件 | 通知正文 |
|---|---|---|
| 对话完成 | `turn/end` + `completed` | 助手回复摘要 |
| 对话中断 | `turn/end` + `aborted` / `interrupted` / `error` / `blocked` / `max-tokens` | 中断、失败或受阻的准确原因 |
| 等待人工选择 | `tool/call` + `ask_user_question` | `等待你的选择` + 第一条问题 |
| 等待授权 | `approval/asked` | `等待授权` + 请求的工具名 |

人工选择和授权分别由 DSH 的正式事件表达。用户提交选择后的 `tool/result`、授权后的 `approval/decided` 会清除壳内等待状态；同一个请求在一次 App 运行中只通知一次。
